...
mixin SectionsMixin {
      ...
      child: Column(
        children: [
          AppBarMixin.buildTitleBar(context, title: 'Build Course', buttons: [
            CustomButtons.customOutlineButton(context, icon: Icons.add, text: 'Create Module', onPressed: () {
              if (courseId != null) {
                ...
                openFailureToast(context, 'Save the course to drafts before creating modules');
              }
            }),
          ]),
          const SizedBox(
  ...
  Widget buildSectionList({
    ...
    if (courseDocId == null) return const Center(child: Text('No modules found'));
  ...
  Widget _buildListItem(
    ...
    return ReorderableDragStartListener(
      ...
      child: ExpansionTile(
        ...
        title: Text('Module ${index + 1} - ${section.name}'),
  ...
  void _onDeleteSection(context, String courseDocId, String sectionId, WidgetRef ref) async {
    ...
    if (UserMixin.hasAccess(ref.read(userDataProvider))) {
      CustomDialogs.openActionDialog(
        ...
        message: 'Warning: All of the classes releated to this module will be deleted and this can not be undone!',
        onAction: () async {
